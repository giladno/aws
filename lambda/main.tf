...
resource "aws_lambda_function" "main" {
  ...
  # Reserved concurrency (only set if explicitly configured, avoid setting -1 which can cause issues)
  reserved_concurrent_executions = local.effective_reserved_concurrency
